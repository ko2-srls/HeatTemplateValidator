...
def cron_generator(crontabs):
    ...
    for cron in crontabs:
        file = open('./htvalidator/list_cron.txt', 'a+')
        file.write("{}\n".format(cron))
        file.close()
...
onlyfiles = [f for f in listdir("./htvalidator/rc_files") if isfile(join("./htvalidator/rc_files", f))]
...
def direct_cron_gen():
    for shfile in onlysh:
        ...
        with open("./htvalidator/rc_files/{0}".format(shfile), 'rt') as F:
            data = F.readlines()
            passwd = ""
            for line in data:
